import { YouTubeAutomationSpec } from '../types';

export const generateQuickCastSpec = (): YouTubeAutomationSpec => {
    return {
        project_constraints: {
...
export const generateMicroSaaSSpec = (projectName: string): string => {
    return `
...
export const generateAIToolSpec = (projectName: string): string => {
    return `
...
export const generatePairProgrammingSpec = (projectName: string): string => {
    return `
...
export const generatePRD = (projectName: string, projectType: string): string => {
    const date = new Date().toLocaleDateString();
    return `# Product Requirements Document (PRD)
**Project Name:** ${projectName}
**Version:** 1.0.0 (MVP Draft)
**Status:** Draft
**Date:** ${date}

## 1. Introduction
This document outlines the requirements for the Minimum Viable Product (MVP) of ${projectName}. The goal is to define a clear scope that can be executed by a solo developer within a short timeframe.

## 2. Objectives
- **Primary Goal:** Validate the core value proposition with minimal engineering effort.
- **Success Criteria:** Launch a functional application that allows a user to complete the core workflow end-to-end.

## 3. User Stories (MVP)

### Core Workflow
- **As a user**, I want to [Core Action related to ${projectType}], so that I can [Benefit].
- **As a user**, I want to see a clear result/output, so that I know the system worked.

### Onboarding & Access
- **As a user**, I want to easily access the tool (web or local), so that I don't waste time on setup.
- **As a user**, I want clear error messages if something goes wrong, so I know how to fix it.

## 4. Functional Requirements

### 4.1 User Interface (UI)
- The interface shall be simple and clean, prioritizing functionality over complex animations.
- Critical actions (e.g., "Generate", "Submit") shall be visually prominent.
- The application must be responsive (work on desktop and potentially mobile).

### 4.2 System Logic
- Input validation must occur before processing data (e.g., ensure text fields are not empty).
- The system shall handle API failures gracefully (e.g., retry logic or user notification).

## 5. Non-Functional Requirements

### 5.1 Performance
- Application load time should be under 2 seconds on standard 4G networks.
- Core processing (excluding external AI latency) should be near-instant.

### 5.2 Security
- API Keys must not be hardcoded in public repositories.
- User input should be sanitized to prevent basic injection attacks (XSS/SQLi).

### 5.3 Reliability
- The application should have a basic uptime of 99% during testing hours.
- Critical errors should be logged to the console or a monitoring service.

## 6. Constraints & Assumptions
- **Solo Developer:** The solution must be maintainable by one person.
- **Budget:** Minimal operating costs (leverage free tiers of Vercel, Supabase, etc.).
- **Timeline:** MVP delivery within the specified 2-4 week cycle.

## 7. Open Questions
- [ ] Are there specific legal compliances (GDPR/CCPA) needed for the MVP?
- [ ] What is the specific budget cap for third-party APIs?
`;
};

export const generateQuickCastPrompt = (): string => {
...
export const generateMicroSaaSPrompt = (): string => {
    return `
...
export const generateAIToolPrompt = (): string => {
    return `
...
export const generatePairProgrammingPrompt = (): string => {
...
export const generateSoloPhasePrompts = (): Record<string, string> => {
    return {
...
export const generateQuickCastStructure = (): string => {
    const structure = {
        directories: {
...
export const generateMicroSaaSStructure = (): string => {
    return `
/ (root)
|-- /src
|   |-- /app
...
export const generateAIToolStructure = (): string => {
    const structure = {
        directories: {
            'src': ['services'],
            'public': []
...
export const generatePairProgrammingStructure = (): string => {
    const structure = {
        directories: {
            'src': [],
            'tests': [],
